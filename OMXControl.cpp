  ...
  else if (dbus_message_is_method_call(m, DBUS_INTERFACE_PROPERTIES, "Get"))
  {
    ...
    else if (strcmp(interface, OMXPLAYER_DBUS_INTERFACE_PLAYER)==0)
    {
      ...
      else if (strcmp(property, "Rate")==0)
      {
        ...
        dbus_respond_double(m, reader->GetSpeed()/1000.);
        return KeyConfig::ACTION_BLANK;
      }
    ...
    if (strcmp(interface, OMXPLAYER_DBUS_INTERFACE_PLAYER)==0)
    {
      ...
      else if (strcmp(property, "Rate")==0)
      {
        ...
        if(rate<MIN_RATE/1000.)
        {
          ...
          dbus_respond_double(m, clock->OMXPlaySpeed()/1000.);
        ...
        if(iSpeed > MAX_RATE)
          iSpeed=MAX_RATE;
        dbus_respond_double(m, iSpeed/1000.);//Reply before applying to be fatser
        reader->SetSpeed(iSpeed);
        audio->SetSpeed(iSpeed);
